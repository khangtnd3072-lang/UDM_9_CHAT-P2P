...
class P2PPeer:
    """
    Một Peer trong mạng P2P.

    Attributes:
        peer_name: Tên của Peer (vd: "Alice", "Bob")
        listen_host: IP để listening (vd: "127.0.0.1")
        listen_port: Port để listening (vd: 5000)
        peers: Dict[peer_name] -> (socket, crypto)
        peers_lock: Lock cho thread-safe access
        use_secure: Có dùng E2EE không
    """
    ...
    def __init__(self, peer_name: str, listen_host: str = "127.0.0.1", listen_port: int = 5000, secure: bool = True):
        ...
        # Track active threads để cleanup khi shutdown
        self.active_threads: set = set()
        self.threads_lock = threading.Lock()

        logger.info(f"✓ Khởi tạo P2PPeer: {peer_name} @ {listen_host}:{listen_port} (secure={secure})")

    def start_listening(self):
        """Khởi động server để accept kết nối từ peer khác"""
        thread = threading.Thread(target=self._listen_for_peers, daemon=True, name=f"{self.peer_name}-Listener")
        thread.start()
    ...
    def _listen_for_peers(self):
        ...
        self.server_socket.settimeout(1.0)  # Cho phép kiểm tra server_running định kỳ
            ...
            while self.server_running:
                try:
                    ...
                    thread = threading.Thread(
                        ...
                        args=(conn, addr),
                        daemon=True,
                        name=f"{self.peer_name}-Handler-{addr[0]}:{addr[1]}"
                    )
                    thread.start()

                    # Track thread để join khi shutdown
                    ...
                    if self.server_running:  # Chỉ log lỗi nếu server còn đang chạy
    ...
    def _cleanup_server(self):
        ...
        logger.info(f"[SERVER] {self.peer_name} server closed")
    ...
    def _handle_incoming_peer(self, conn: socket.socket, addr: tuple):
        ...
        try:
            ...
            self._cleanup_peer_connection(peer_name, conn, addr)

    def _cleanup_peer_connection(self, peer_name: Optional[str], conn: socket.socket, addr: tuple):
    ...
    def connect_to_peer(self, peer_name: str, peer_host: str, peer_port: int) -> bool:
        ...
        try:
            ...
            thread = threading.Thread(
                ...
                daemon=True,
                name=f"{self.peer_name}-Receiver-{peer_name}"
            )
            thread.start()
    ...
    def _receive_from_peer(self, peer_name: str, sock: socket.socket, peer_crypto: CryptoManager):
        ...
        try:
            while True:
                # QUAN TRỌNG: bọc try/except socket.timeout NGAY TRONG vòng lặp,
                # kể cả ở chế độ secure — nếu không, một lần timeout 1s không có
                # tin nhắn tới sẽ làm thread này thoát hẳn và "mất" kết nối
                # dù socket vẫn còn sống (bug đã có ở bản trước khi gộp).
                try:
                    if self.use_secure:
                    ...
                    # Timeout bình thường (chưa có tin nhắn mới) - tiếp tục chờ
                    continue

        except EOFError:
            # Peer đóng kết nối gracefully
            ...
            logger.info(f"[RECV] Đóng receiver thread cho {peer_name}")
    ...
    def shutdown(self):
        ...
        with self.peers_lock:
            for peer_name, (sock, _) in list(self.peers.items()):
                try:
                    sock.shutdown(socket.SHUT_RDWR)
                except:
                    pass
        ...
        # Chờ tất cả threads thoát (với timeout) để tránh treo/rò rỉ thread
        logger.info(f"[SHUTDOWN] Chờ {len(self.active_threads)} thread thoát...")
...
def main():
    ...
    parser.add_argument("--name", required=True, help="Tên của Peer (vd: Alice, Bob)")
